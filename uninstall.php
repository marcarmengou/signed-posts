...
$delete_user_dids = get_option( 'wppgps_delete_user_did_identifiers', '0' ) === '1';
...
if ( $delete_post_sigs && function_exists( 'delete_post_meta_by_key' ) ) {
    ...
    delete_post_meta_by_key( '_wppgps_sig_method' );
}
...
// Remove all stored DID identifiers from user meta if requested
if ( $delete_user_dids ) {
    delete_metadata( 'user', 0, 'did_identifier', '', true );
}

// Clean our own options
delete_option( 'wppgps_delete_post_signatures' );
delete_option( 'wppgps_delete_user_key_urls' );
delete_option( 'wppgps_delete_user_did_identifiers' );
